...
import { Box, TabPanel, TabStepper, type TabKey, type TabStepItem } from '@orfium/ictinus';
import { getContent } from './constants';

type Status = 'pending' | 'warning' | 'done';

type StoryArgs = Parameters<typeof TabStepper>[0] & {
  status: Status;
  status1: Status;
  status2: Status;
  status3: Status;
...
export const TabStepperTypes: Story = {
  render: () => {
    const [key1, setKey1] = useState<TabKey>('step_1');
    const [key2, setKey2] = useState<TabKey>('step_1');
...
export const TabStepperWithContent: Story = {
  render: () => {
    const [key, setKey] = useState<TabKey>('step_1');
...
export const TabStepperDisabled: Story = {
  render: () => {
    const [key, setKey] = useState<TabKey>('tab-1');
