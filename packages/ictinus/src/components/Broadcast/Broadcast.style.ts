...
const broadcastStyles = (props: BroadcastProps) => css`
  ...
  border: var(--_border-width) solid ${getBorderColor(props.status)};
...
const getBorderColor = (status: BroadcastProps['status']) => {
  switch (status) {
    case 'informational':
...
export const getIconColor = (status: BroadcastProps['status']) => {
  switch (status) {
    case 'informational':
      return vars.color.indicators.brand;
    case 'error':
