import { forwardRef } from 'react';
import { IconPrimitive, type IconPrimitiveProps } from './IconPrimitive';

export const AlbumIcon = forwardRef<SVGSVGElement, IconPrimitiveProps>((props, ref) => (
  <IconPrimitive ref={ref} {...props}>
    <svg viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M9.99986 7.6001C8.6799 7.6001 7.59994 8.68006 7.59994 10C7.59994 11.32 8.6799 12.3999 9.99986 12.3999C11.3198 12.3999 12.3998 11.32 12.3998 10C12.3998 8.68006 11.3198 7.6001 9.99986 7.6001ZM10 2C5.5924 2 2 5.5924 2 10C2 14.4076 5.59163 18 10 18C14.4076 18 18 14.4084 18 10C18 5.5924 14.4076 2 10 2ZM9.99994 13.9996C7.79201 13.9996 6.00033 12.2079 6.00033 10C6.00033 7.79208 7.79201 6.00039 9.99994 6.00039C12.2079 6.00039 13.9996 7.79208 13.9996 10C13.9996 12.2079 12.2079 13.9996 9.99994 13.9996Z"
        fill="currentColor"
      />
    </svg>
  </IconPrimitive>
));

AlbumIcon.displayName = 'AlbumIcon';
