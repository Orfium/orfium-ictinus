...
function deepTransform(node, transform) {
  ...
  if (transformed !== node && transformed.children) {
    ...
    return copyProps(transformed, {
      ...
      children: flatMap(deepTransform(transformed.children, transform), (id) => id),
    });
  }
...
function filterEmotionProps(props = {}) {
  const { css, __EMOTION_TYPE_PLEASE_DO_NOT_USE__, __EMOTION_LABEL_PLEASE_DO_NOT_USE__, ...rest } =
    props;
...
function isShallowEnzymeElement(element: any, keys: string[], labels: string[]) {
...
export function createSerializer({
  ...
  function serialize(
    val: { wrapperHTML: string; innerHTML: string },
    config,
    indentation: string,
    depth: number,
    refs,
    ...
    const { document } = new JSDOM(`<!DOCTYPE html>${val.wrapperHTML}`).window;
    const { document: innerDocument } = new JSDOM(val.innerHTML).window;

    const elements = getStyleElements(document);
    const keys = getKeys(elements);
    const convertEmotionElements = createConvertEmotionElements(keys);
    const converted = deepTransform(innerDocument.getRootNode().firstChild, convertEmotionElements);
