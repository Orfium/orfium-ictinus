import { Meta, Canvas, ArgTypes, Story } from '@storybook/addon-docs';
import { boolean, select, text, withKnobs } from '@storybook/addon-knobs';
import DatePicker from './DatePicker';
import { CALENDAR_DEFAULT_OPTIONS } from './constants';
import { openDatePicker, currentDay } from './utils';
import {
  rangePickerOptions,
  filterTypeOptions,
  disableDates,
  options,
  dateFormatOptions,
} from './constants';
import Stack from '../storyUtils/Stack';
import { FIGMA_URL, Function } from '../../utils/common';
import SectionHeader from '../../storybook/SectionHeader';
import { useState } from 'react';

<Meta
  title="Design System/DatePicker"
  component={DatePicker}
  parameters={{
    design: [
      {
        type: 'figma',
        name: 'Dates',
        url: `${FIGMA_URL}?node-id=1637%3A37861`,
      },
      {
        type: 'figma',
        name: 'DatePicker',
        url: `${FIGMA_URL}?node-id=10283%3A104356`,
      },
    ],
  }}
/>

<SectionHeader title={'DatePicker'} />

- [Overview](#overview)
- [Props](#props)
- [Usage](#usage)
- [Variants](#variants)

## Overview

A universal DatePicker component that allows the user to select a single date or a range of dates.
User may either type the date in the suggested format (text input) or click on the ‘calendar’ icon to the right to open the date picker modal

## Props

<ArgTypes of={DatePicker} />

## Usage

```js
import { DatePicker } from '@orfium/ictinus';

<DatePicker disableFutureDates={false} disableFutureDates={false} onChange={console.log} />;
```

## Variants

### DatePicker

Implementation of the regular DatePicker

<Canvas name="DatePicker" parameters={{ decorators: [withKnobs] }} play={openDatePicker} autoPlay>
  <Stack vertical height={600}>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            value={date}
            onChange={setDate}
            isClearable={true}
            disableDates={select('disableDates', options, options[0])}
            dateFormatOverride={select('dateFormat', dateFormatOptions, dateFormatOptions[1])}
          />
        );
      }}
    </Function>
  </Stack>
</Canvas>

### DateRangePicker

Implementation of the regular DateRangePicker

<Canvas
  name="DateRangePicker"
  parameters={{ decorators: [withKnobs] }}
  play={openDatePicker}
  autoPlay
>
  <Stack vertical height={600}>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            isRangePicker
            value={date}
            onChange={setDate}
            isClearable={true}
            disableDates={select('disableDates', options, options[0])}
            dateFormatOverride={select('dateFormat', dateFormatOptions, dateFormatOptions[1])}
          />
        );
      }}
    </Function>
  </Stack>
</Canvas>

### DatePicker and DateRangePicker with Statuses

DatePicker and DateRangePicker inherit all TextField statuses behavior.

<Canvas
  name="DatePicker and DateRangePicker with Statuses"
  parameters={{ decorators: [withKnobs] }}
>
  <Stack vertical height={600}>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            value={date}
            isRangePicker={boolean('isRangePicker', false)}
            onChange={setDate}
            isClearable={true}
            disableDates={select('disableDates', options, options[0])}
            dateFormatOverride={select('dateFormat', dateFormatOptions, dateFormatOptions[1])}
            inputProps={{
              label: 'Normal',
              status: {
                type: 'normal',
                hintMessage: text('Custom Hint Message', 'Hint Message'),
              },
            }}
          />
        );
      }}
    </Function>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            value={date}
            isRangePicker={boolean('isRangePicker', false)}
            onChange={setDate}
            isClearable={true}
            disableDates={select('disableDates', options, options[0])}
            dateFormatOverride={select('dateFormat', dateFormatOptions, dateFormatOptions[1])}
            inputProps={{
              label: 'Error',
              status: {
                type: 'error',
                hintMessage: text('Custom Error Message', 'Error Message'),
              },
            }}
          />
        );
      }}
    </Function>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            value={date}
            isRangePicker={boolean('isRangePicker', false)}
            onChange={setDate}
            isClearable={true}
            disableDates={select('disableDates', options, options[0])}
            dateFormatOverride={select('dateFormat', dateFormatOptions, dateFormatOptions[1])}
            inputProps={{
              label: 'Read-only',
              status: {
                type: 'read-only',
                hintMessage: text('Custom Hint Message', 'Hint Message'),
              },
            }}
          />
        );
      }}
    </Function>
  </Stack>
</Canvas>

### DateRangePicker with options

Implementation of the regular DatePicker with options

<Canvas name="DateRangePicker with options" parameters={{ decorators: [withKnobs] }}>
  <Stack vertical height={600}>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            isRangePicker
            value={date}
            onChange={setDate}
            options={CALENDAR_DEFAULT_OPTIONS}
            isClearable={true}
            disableDates={select('disableDates', options, options[0])}
            dateFormatOverride={select('dateFormat', dateFormatOptions, dateFormatOptions[1])}
          />
        );
      }}
    </Function>
  </Stack>
</Canvas>

### DatePicker with Filter

Implementation of the DatePicker with a Filter instead of a TextField

<Canvas name="DatePicker with Filter" parameters={{ decorators: [withKnobs] }}>
  <Stack vertical height={600}>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: undefined, to: undefined });
        return (
          <DatePicker
            isRangePicker={select('isRangePicker', rangePickerOptions, rangePickerOptions[0])}
            filterConfig={{
              filterType: select('filterType', filterTypeOptions, filterTypeOptions[0]),
            }}
            value={date}
            onChange={setDate}
          />
        );
      }}
    </Function>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            value={date}
            onChange={setDate}
            isRangePicker={select('isRangePicker', rangePickerOptions, rangePickerOptions[0])}
            filterConfig={{
              filterType: select('filterType', filterTypeOptions, filterTypeOptions[0]),
            }}
          />
        );
      }}
    </Function>
  </Stack>
</Canvas>

### DatePicker with disabled dates

Some disable dates functionality

<Canvas name="DatePicker with disabled dates">
  <Stack height={600}>
    <Function>
      {() => {
        const [date, setDate] = useState({
          from: currentDay.toDate(),
          to: currentDay.toDate(),
        });
        return <DatePicker disableDates={disableDates} value={date} onChange={setDate} />;
      }}
    </Function>
  </Stack>
</Canvas>

### Disabled DatePicker & DateRangePicker

Disabled DatePicker & DateRangePicker

<Canvas name="Disabled DatePicker & DateRangePicker">
  <Stack>
    <Function>
      {() => {
        const [date, setDate] = useState({ from: currentDay.toDate(), to: currentDay.toDate() });
        return (
          <DatePicker
            value={date}
            isRangePicker={boolean('isRangePicker', false)}
            onChange={setDate}
            isClearable={true}
            inputProps={{
              label: 'Normal',
              status: {
                type: 'normal',
                hintMessage: 'This field is disabled',
              },
              isDisabled: true,
            }}
          />
        );
      }}
    </Function>
  </Stack>
</Canvas>

### Playground

<Canvas name="Playground" parameters={{ decorators: [withKnobs] }}>
  <Function>
    {() => {
      const [date, setDate] = useState({
        from: currentDay.toDate(),
        to: currentDay.toDate(),
      });
      return (
        <DatePicker
          disableDates={select('disableDates', options, options[0])}
          inputProps={{
            label: text('label', 'Custom Date'),
            status: {
              type: select('status', ['normal', 'error', 'read-only'], 'normal'),
              hintMessage: text('hintMessage', ''),
            },
          }}
          isRangePicker={boolean('isRangePicker', false)}
          value={date}
          onChange={setDate}
        />
      );
    }}
  </Function>
</Canvas>
