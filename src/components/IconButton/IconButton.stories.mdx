import { Meta, Preview, Props, Story } from '@storybook/addon-docs';
import { boolean, select } from '@storybook/addon-knobs';
import IconButton from './IconButton';
import Stack from '../storyUtils/Stack';
import iconSelector from '../Icon/assets/iconSelector';
import { colorShades, flatColors, mainTypes } from '../../theme/palette';
import { FIGMA_URL } from '../../utils/common';

<Meta
  title="Design System/Button/IconButton"
  component={IconButton}
  parameters={{
    design: [
      {
        type: 'figma',
        name: 'Buttons',
        url: `${FIGMA_URL}?node-id=574%3A7239`,
      },
      {
        type: 'figma',
        name: 'Documentation',
        url: `${FIGMA_URL}?node-id=8167%3A146863`,
      },
    ],
  }}
/>

# IconButton

A universal IconButton component that with the help of Button hold the icon inside it and makes it clickable and easily accessible.

# Usage

```js
import { IconButton } from '@orfium/ictinus';

<IconButton onClick={() => {}} name={'add'} />;
```

# Props

<Props of={IconButton} />

# IconButton Types

IconButton with different types `primary, secondary, tertiary` and different radius types `rounded, squared`

<Preview>
  <Story name="IconButton Types">
    <Stack>
      <IconButton name="dotsVertical" />
      <IconButton name="dotsVertical" type="secondary" />
      <IconButton name="dotsVertical" type="tertiary" />
    </Stack>
    <Stack>
      <IconButton name="dotsVertical" radius="squared" />
      <IconButton name="dotsVertical" type="secondary" radius="squared" />
      <IconButton name="dotsVertical" type="tertiary" radius="squared" />
    </Stack>
  </Story>
</Preview>

# IconButton Playground

<Preview>
  <Story name="IconButton Playground">
    <Stack>
      <IconButton
        name={select(
          'name',
          Object.keys(iconSelector)
            .sort((a, b) => a.localeCompare(b))
            .map((iconName) => iconName),
          'check'
        )}
        type={select('type', ['primary', 'secondary', 'tertiary'], 'primary')}
        radius={select('radius', ['rounded', 'squared'], 'rounded')}
      />
    </Stack>
  </Story>
</Preview>
