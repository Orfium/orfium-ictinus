...
<Preview>
  <Story name="Pagination" parameters={{ decorators: [withKnobs] }}>
    <Pagination count={3}
                page={1}
                nextPageDisabled={boolean('nextPageDisabled', false)}
                prevPageDisabled={boolean('prevPageDisabled', false)}
                arrowColor={select('arrowColor', ['#f14949', '#c2f740', '#283abe', 'primary', 'darkGray'], 'darkGray')}/>
    />
  </Story>
</Preview>
