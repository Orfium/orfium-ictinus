...
<Tip>
  ...
  also have a `sticky` optional prop.
</Tip>
...
### Fixed and custom content

The drawer content consists of `header`, `body` and `footer`. These 3 modules are all options and also have a `sticky` optional prop.

<Tip>
  In order to create your own custom drawer content with custom styles, you can only user the body
...
<Tip>
  ...
  own close button based on your custom content layout, either skip the header content entirely or
