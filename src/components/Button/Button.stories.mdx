...
<Meta title="Design System/Button" component={Button} parameters={{
  design: [{
    type: 'figma',
    name: 'Button',
    url: `${FIGMA_URL}?node-id=9660%3A117281`
  },{
    type: 'figma',
    name: 'Anatomy',
...
<Preview>
  <Story name="Dynamic Button Props" parameters={{ decorators: [withKnobs] }}>
    <Stack vertical>
      ...
      <Button filled={false} size={'sm'} iconLeft={<Icon name="close" size={16} color='black' />}>Hello world</Button>
      <Button filled={false} size={'sm'}>Hello world</Button>
    </Stack>
  </Story>
</Preview>
