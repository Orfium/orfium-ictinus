...
export function findLast<T>(arr: T[], predicate: (arg0: T) => boolean) {
...
export function findIndexFrom<T>(arr: T[], fromIndex: number, predicate: (arg0: T) => boolean) {
...
export function unwrapFromPotentialFragment(node: any) {
...
function getClassNamesFromEnzyme(selectors, nodeWithPotentialFragment) {
  ...
  const nodeWithClassName = findNodeWithClassName(isShallow ? node.dive() : node);
...
export function isReactElement(val: any): boolean {
  return (
    val.$$typeof === Symbol.for('react.test.json') || val.$$typeof === Symbol.for('react.element')
  );
}
...
export function getStylesFromClassNames(
  ...
  const targetClassName = classNames.find((className) => /^e[a-z0-9]+$/.test(className));
  const keyPattern = `(${keys.join('|')})-`;
  const classNamesRegExp = new RegExp(
    targetClassName ? `^(${keyPattern}|${targetClassName})` : `^${keyPattern}`
  );
  const filteredClassNames = classNames.filter((className) => classNamesRegExp.test(className));
  ...
  let styles = rules
    ...
    .filter(Boolean)
    .sort(([ruleA, classNameIndexA], [ruleB, classNameIndexB]) => classNameIndexA - classNameIndexB)
    .map(([rule]) => rule)
    .join('');
...
export function getStyleElements(document): Array<HTMLStyleElement> {
  // if (!isBrowser) {
  //   throw new Error(
